...
- Kali-style: dir_bruteforce (content discovery), vhost_scan, wpscan (WordPress enum), takeover_check (subdomain takeover), s3_check (open buckets), dns_axfr (zone transfer), whois, hash_crack (offline dictionary), inject_probe (LFI/SSTI/command-injection detection PoC via a FUZZ marker), favicon_hash (Shodan pivot)
