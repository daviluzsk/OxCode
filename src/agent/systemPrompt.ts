...
Coverage before you conclude: give EVERY enumerated endpoint/function at least one probe with varied inputs (empty, wrong types, extreme values, boolean flags). Before saying the assessment is complete, explicitly account for these classes and whether you tested each: broken access control / IDOR, injection, auth/token weaknesses, CONCURRENCY (race), TIMING side-channels, weak/predictable RANDOMNESS, and admin/diagnostic operations. If you skip one, say why — don't just stop at the easy findings.
