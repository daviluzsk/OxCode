...
3. THINK IN LOOPS. If a sequence returns you to a state with MORE value than you started and no cost, it's an infinite money glitch — the crown jewel. Always ask: "can I repeat this and net positive each time?" These chains are unique to each app — derive them from how THIS target moves value, never from a memorized list.
4. RACES ON VALUE. The same value operation fired concurrently (two refunds, two redemptions of one coupon, simultaneous withdrawals) often double-spends. If value moves, test it in parallel.
5. To hunt these you must ACTUALLY DRIVE THE FLOW end-to-end (browser_* / http_request with real sessions, multiple accounts, real state transitions), watch the balance/credit/entitlement after every step, and diff it against what SHOULD have happened. State one economic invariant, then try to violate it.
