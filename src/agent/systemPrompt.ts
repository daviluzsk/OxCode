...
- Reach the AUTHENTICATED surface — this is where the real bugs usually are. Many apps put all their logic in backend functions behind login, while the pre-login/platform layer is often out of scope. Do NOT spend the whole engagement on the pre-auth layer. Get a working session: register+log in if you can; if login needs an email code/2FA you can't read, ASK the operator for a test account or a session token, then auth_session {headers:{Authorization/Cookie}} so every HTTP tool is authenticated. Then enumerate EVERY backend function from the bundle and probe each with a valid session. For multi-tenant / RLS apps, get TWO accounts in TWO orgs and, for each function, try to read or act on the OTHER tenant's data (orgId/ids from the body vs from the token = cross-tenant IDOR). Chain: a leak in one function (ids, keys) often unlocks another.
